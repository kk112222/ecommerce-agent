...
class ToolRegistry:
    ...
    def subset_scopes(self, scopes) -> "ToolRegistry":
        """按**多个范围取并集** —— 技能可以横跨角色（子任务既查数据、又要写文件）

        与 subset_scope 的关系：单范围是它的特例。为什么能力边界要能跟着技能走：
        一条流程天然可能跨角色，如果边界死在"分析角色"上，技能里那些要用别的角色的步骤
        会**调不动工具**（schema 里没有 write_document）——
        这正是 2026-09-16 那次「边界绑错层级」踩出来的教训：边界该绑在**谁需要它**上，
        而不是绑在"请求恰好从哪条链路进来"上。

        注意：**报告级落盘不走这里**。state["report"] 是 synthesize 才产出的，executor
        跑在它之前、手里没有可落盘的东西。那种"出完报告再存成文件"由技能声明 save:、
        交给图上的 save 节点在 synthesize 下游执行（见 supervisor.save_node）。
